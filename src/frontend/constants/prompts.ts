...
export const PROMPT_SECTIONS = {
  // Qm 工具（且慢）提示词
  qm: {
    base: `- 只能通过MCP \`qm\`（且慢）对我进行询问，禁止直接询问或结束任务询问`,
    detail: `且慢工具（qm）使用细节：
- 需求不明确时使用 \`qm\` 询问澄清，提供预定义选项
- 在有多个方案的时候，需要使用 \`qm\` 询问，而不是自作主张
- 在有方案/策略需要更新时，需要使用 \`qm\` 询问，而不是自作主张
- 即将完成请求前必须调用 \`qm\` 请求反馈
- 在没有明确通过使用 \`qm\` 询问并得到可以完成任务/结束时，禁止主动结束对话/请求`,
  } as PromptSection,
