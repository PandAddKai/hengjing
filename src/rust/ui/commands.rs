use crate::config::{
    ...
    ShortcutBinding, ShortcutConfig, TimeoutAutoSubmitConfig, WindowConfig,
};
...
#[tauri::command]
pub async fn get_timeout_auto_submit_config(
    state: State<'_, AppState>,
) -> Result<TimeoutAutoSubmitConfig, String> {
    let config = state
        .config
        .lock()
        .map_err(|e| format!("获取配置失败: {}", e))?;
...
#[tauri::command]
pub async fn get_current_window_size(
    window: tauri::WebviewWindow,
) -> Result<serde_json::Value, String> {
    // 检查窗口是否最小化
